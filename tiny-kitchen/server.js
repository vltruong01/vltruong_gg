...
const io = new Server(server, {
  // Prefer websocket to reduce overhead; keep polling fallback.
  transports: ["websocket", "polling"],
  // On small CPUs, permessage-deflate can cost more than it saves.
...
function startGame(room, lockedCount) {
  ...
  room.game = game;
  // Send static layout once (clients cache it)
  emitStaticToRoom(room);
...
function updateRoom(room) {
  ...
  if (t - game.lastSnapshotAt >= (1000 / SNAPSHOT_RATE)) {
    ...
    io.to(room.code).volatile.emit("state", buildDynamicSnapshot(room));
  }
}


// ===== Snapshot splitting (static vs dynamic) =====
// Static layout rarely changes; sending it on every snapshot wastes CPU & bandwidth.
// We send it once via event "static" whenever layout changes (start game / play again / join mid-game).

function buildStaticSnapshotFromLayout(layout) {
  return {
    ...
    stations: layout.stations.map(s => ({
      ...
      // For stacks/oven, client needs to know how many slots to draw.
      slotCount: s.type === "PLATE" ? Number(s.slotCount || 0) :
                 s.type === "OVEN" ? (Array.isArray(s.slotItemIds) ? s.slotItemIds.length : 2) :
                 (s.type === "SINK" ? 2 : 0)
    }))
  };
}
...
function ensureStatic(room) {
  // Build a layout to represent current visual state:
  // - playing: use game.layout
  // - lobby: build a default layout (3 slots, 2 players)
  const layout = room.game ? room.game.layout : buildKitchenLayout(3, room.lockedCount || 2);
  const key = JSON.stringify({ phase: room.phase, lockedCount: room.lockedCount || 2 });
  if (!room.staticSnapshot || room.staticKey !== key) {
    room.staticKey = key;
    room.staticVersion = (room.staticVersion || 0) + 1;
    room.staticSnapshot = buildStaticSnapshotFromLayout(layout);
  }
  return room.staticSnapshot;
}

function emitStaticToRoom(room) {
  ensureStatic(room);
  io.to(room.code).emit("static", { version: room.staticVersion || 1, ...room.staticSnapshot });
}

function emitStaticToSocket(socket, room) {
  ensureStatic(room);
  socket.emit("static", { version: room.staticVersion || 1, ...room.staticSnapshot });
}

function buildStationStates(layout, game) {
  const out = {};
  if (!layout) return out;

  for (const s of layout.stations) {
    if (s.type === "OVEN") {
      const ids = Array.isArray(s.slotItemIds) ? s.slotItemIds : [];
      out[s.id] = {
        type: "OVEN",
        ...
        slotTs: Array.isArray(s.slotTs) ? s.slotTs : []
      };
    } else if (s.type === "SINK") {
      ...
      out[s.id] = {
        ...
        slotTs: Array.isArray(s.slotTs) ? s.slotTs : [],
        slotActives: Array.isArray(s.slotActives) ? s.slotActives : [],
        slotByPlayerIds: Array.isArray(s.slotByPlayerIds) ? s.slotByPlayerIds : []
      };
    }
  }
  return out;
...
function buildDynamicSnapshot(room) {
  const game = room.game;
  const cfg = room.config;
  const layout = game ? game.layout : buildKitchenLayout(3, room.lockedCount || 2);

  const playersOut = Array.from(room.players.values()).map(p => {
    let heldType = null, heldMeta = null, heldPlate = null;
    if (game && p.heldItemId) {
      const it = game.items.find(it => it.id === p.heldItemId);
      if (it) {
        heldType = it.type;
        heldMeta = it.meta || null;
        if (it.type === "PLATE") heldPlate = it.plate || null;
      }
    }
    return { id: p.id, name: p.name, x: p.x, y: p.y, connected: p.connected, heldType, heldMeta, heldPlate };
  });

  const itemsOut = game ? game.items.map(it => ({
    id: it.id,
    type: it.type,
    x: it.x, y: it.y,
    zone: it.zone,
    meta: it.meta || null,
    plate: it.type === "PLATE" ? (it.plate || null) : null
  })) : [];

  const customersOut = game ? (game.customers || []).map(c => {
    // Keep existing customer export fields (already optimized)
    // This block mirrors the original snapshot format.
    const inWait = c.state === "WAIT_FOOD";
    const inGreetMain = c.state === "WAIT_MAIN";
    const inGreetGroup = c.state === "WAIT_SIDE";
    const g = c.side || null;
  ...
  return {
    ...
    hostId: room.hostId,
    lockedCount: room.lockedCount || null,
    staticVersion: room.staticVersion || 1,
    ...
    game: game ? {
      ...
      score: game.score,
      target: game.target,
      below2Since: game.below2Since,
      paused: !!game.paused
    } : null,

    config: cfg || null,
    stationStates: game ? buildStationStates(layout, game) : {},
    customers: customersOut,
    players: playersOut,
    items: itemsOut,
    centerSlots: game ? game.centerSlots : null,
    plateStacks: game ? game.plateStacks : null,
    // include these if client uses them
    messages: game ? game.messages : null
  };
}
...
io.on("connection", (socket) => {
  ...
  socket.on("createRoom", ({ name }) => {
    ...
    emitStaticToSocket(socket, room);
    if (room.phase === "playing") {
      socket.emit("state", buildDynamicSnapshot(room));
    }
    roomBroadcast(room);
  });
